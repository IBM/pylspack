...
def execute_and_check(alpha: float, A: np.ndarray, B: np.ndarray, beta: float, C: np.ndarray):
    atol = eps_machine * A.shape[1] * A.shape[1] * B.shape[1]
...
def test_rmsqn(A_shape, B_shape):
    A, B, C = get_random_matrices(A_shape, B_shape, (A_shape[0], ))
    run_all(A, B, C)
    set_arrays_elements_to_value(A, B, C, value=0)
    run_all(A, B, C)
    set_arrays_elements_to_value(A, B, C, value=1)
    run_all(A, B, C)
