...
import pytest
from pylspack.linalg_kernels import csrcgs

density = [0.05, 0.1, 0.3, 1]
csrcgs_matrices = [
    random(_m, _n, density=_density, format='csr')
    for _m, _n in zip([783, 4169, 9178, 17633], [2, 17, 31, 49]) for _density in density[1:]
]


@pytest.mark.parametrize('A', csrcgs_matrices)
def test_csrcgs(A):
    n, d = A.shape
    for gamma in [0, 2]:
        m = gamma * d
        ...
        _, singvals_A, _ = svd(A.toarray(), full_matrices=False)
